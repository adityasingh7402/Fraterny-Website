...
import { getDuplicateGroupUsers, mergeDuplicateUsers, mergeDuplicateUsersWithCustomPrimary } from '@/services/admin-users';
import type { DuplicateGroup, UserData } from '@/services/admin-users';
...
const DuplicateManagementPopup: React.FC<DuplicateManagementPopupProps> = ({
  ...
  const handleMergeConfirmation = async () => {
    ...
    setMerging(true);
    try {
      // Create a modified group with the selected primary user
      const selectedPrimary = duplicateGroup.users.find(u => u.user_id === selectedPrimaryUserId);
      const otherUsers = duplicateGroup.users.filter(u => u.user_id !== selectedPrimaryUserId);
      
      if (!selectedPrimary) {
        toast.error('Selected primary user not found');
        return;
      }
      
      const modifiedGroup = {
        ...duplicateGroup,
        primaryUser: selectedPrimary,
        duplicateUsers: otherUsers,
        users: [selectedPrimary, ...otherUsers] // Primary first
      };
      
      // For now, we'll use the existing merge function but we need to create a custom merge
      // that uses the selected primary user
      const result = await mergeDuplicateUsersWithCustomPrimary(groupKey, selectedPrimaryUserId);
