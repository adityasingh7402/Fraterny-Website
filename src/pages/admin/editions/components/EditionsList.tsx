...
import { VillaEdition, deleteEdition, updateEdition } from '@/services/website-settings';
...
const EditionsList: React.FC<EditionsListProps> = ({ editions, onEdit, onRefresh }) => {
  const handleToggleActive = async (edition: VillaEdition) => {
    try {
      const success = await updateEdition(edition.id, { isActive: !edition.isActive });
      if (success) {
  ...
  const handleDelete = async (edition: VillaEdition) => {
    ...
    try {
      const success = await deleteEdition(edition.id);
      if (success) {
              ...
              <tr key={edition.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <button
                    ...
                    className="text-navy hover:text-navy/80 mr-4"
                    title="Edit"
                  >
