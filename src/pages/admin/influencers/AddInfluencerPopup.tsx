...
import { createInfluencer, generateAffiliateCode } from '@/services/admin-influencers';
import type { CreateInfluencerInput, SocialLinks, PaymentInfo } from '@/services/admin-influencers';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profileImageFile) {
        ...
        const imageKey = `influencer-${affiliateCode.toLowerCase()}-${Date.now()}`;
      ...
      const response = await createInfluencer(input);

      if (response.success) {
        toast.success('Influencer created successfully!');
        onSuccess();
      } else {
        toast.error(response.error || 'Failed to create influencer');
      }
    } catch (error: any) {
