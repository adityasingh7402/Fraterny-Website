...
const AdminQuestPayment: React.FC = () => {
  ...
  const fetchStatusCounts = async () => {
    try {
      ...
      const [successResponse, attemptedResponse, disputedResponse] = await Promise.all([
        fetchPaymentDetails('success', { page: 1, pageSize: 1 }),
        fetchPaymentDetails('Start', { page: 1, pageSize: 1 }),
        fetchPaymentDetails('error', { page: 1, pageSize: 1 })
      ]);
  ...
  const fetchPayments = async () => {
    ...
    try {
      ...
      const response = await fetchPaymentDetails(activeStatus, paginationParams, filters);
      ...
      if (response.success && response.data) {
        ...
        console.log('📊 Final transaction data being set:', {
          originalCount: response.data.transactions.length,
          filteredCount: filteredTransactions.length,
          pageSize,
          currentPage,
        ...
        setError(response.error || 'Failed to load payment data');
        setTransactions([]);
        setPagination(null);
      }
    } catch (err: any) {
  ...
  // Process refund (would connect to actual refund API)
  const processRefund = async () => {
    // In a real implementation, you would call your refund API here
    ...
    try {
      ...
      // Then refresh data
      fetchPayments();
      // You might also want to update status counts
      fetchStatusCounts();
    } catch (error) {
      ...
      <div className="layout-container flex h-full grow flex-col">
        <main className="flex-1 px-8 sm:px-6 lg:px-16 py-8">
          <div className="max-w-full mx-auto">
            ...
            {showRefundPopup && selectedTransaction && (
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
                <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
                  <h3 className="text-lg font-bold text-gray-900 mb-4">Confirm Refund</h3>
                  <p className="text-sm text-gray-600 mb-4">
                    Are you sure you want to process refund for transaction #{selectedTransaction.transaction_id || selectedTransaction.payment_id}?
                  </p>
                  <div className="bg-gray-50 p-3 rounded-md mb-4">
                    <p className="font-medium">Refund Amount: <span className="text-red-600">{formatCurrency(selectedTransaction.total_paid, selectedTransaction.gateway, selectedTransaction.IsIndia)}</span></p>
                    <p className="text-xs text-gray-500 mt-1">To: {selectedTransaction.user_data?.user_name || 'Unknown'} ({selectedTransaction.user_data?.email || 'N/A'})</p>
                  </div>
