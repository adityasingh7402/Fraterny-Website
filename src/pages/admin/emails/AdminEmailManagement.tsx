...
const AdminEmailManagement: React.FC = () => {
  ...
  const fetchAdminEmailsData = async () => {
    ...
    try {
      const response = await fetchAllAdminEmails();
      if (response.success && response.data) {
        setAdminEmails(response.data);
      } else {
        setError(response.error || 'Failed to load admin emails');
      }
    } catch (err: any) {
  ...
  const handleAddEmail = async () => {
    ...
    setSaving(true);
    try {
      const response = await addAdminEmail({ email: newEmail.trim() });
      if (response.success) {
        ...
        toast.error(response.error || 'Failed to add admin email');
      }
    } catch (err: any) {
  ...
  const handleToggleStatus = async (id: number, currentStatus: boolean) => {
    try {
      const response = await updateAdminEmailStatus(id, !currentStatus);
      if (response.success) {
        toast.success(`Admin email ${!currentStatus ? 'activated' : 'deactivated'}`);
        await fetchAdminEmailsData();
      } else {
        toast.error(response.error || 'Failed to update admin email');
      }
    } catch (err: any) {
  ...
  const handleDeleteEmail = async (id: number, email: string) => {
    ...
    try {
      const response = await deleteAdminEmail(id);
      if (response.success) {
        toast.success('Admin email deleted successfully');
        await fetchAdminEmailsData();
      } else {
        toast.error(response.error || 'Failed to delete admin email');
      }
    } catch (err: any) {
